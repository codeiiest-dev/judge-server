...
class CachedExecutor(object):
    @classmethod
    def of_executor(cls, executor_clazz):
        ...
        # Contract: if an entry exists in the cache, `create_files` and `compile` will not
        # be run, and `_executable` will be loaded from the cache.
        class CacheMeta(type):
            ...
            def __call__(self, *args, **kwargs):
                is_cached = hasattr(executor_clazz, 'get_binary_cache_key')
                if is_cached:
                    kwargs['dest_dir'] = env.compiled_binary_cache_dir

                # Finish running all constructors before compiling.
                obj = super(CacheMeta, self).__call__(*args, **kwargs)
        ...
        class CachedExecutorWrapper(six.with_metaclass(CacheMeta, executor_clazz)):
            pass

        return CachedExecutorWrapper
