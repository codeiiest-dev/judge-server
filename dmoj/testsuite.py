...
class TestManager:
    ...
    def set_expected(self, codes_all, codes_cases, feedback_all, feedback_cases, extended_feedback_all,
    ...
    def test_case_status_packet(self, position, result):
        ...
        if position in self.extended_feedback_cases:
            feedback = self.extended_feedback_cases[position]
...
class Tester:
    ...
    def run_test_case(self, problem, case, case_dir):
        ...
        feedback_all, feedback_cases = self.parse_expect(config.get('feedback'),
                                                         ...
                                                         self.parse_feedback)
        extended_feedback_all, extended_feedback_cases = self.parse_expect(config.get('extended_feedback'),
                                                                           config.get('extended_feedback_cases', {}),
                                                                           self.parse_feedback)
        ...
        for source in sources:
            self.sub_id += 1
            self.manager.set_expected(codes_all, codes_cases, feedback_all, feedback_cases, extended_feedback_all,
                                      extended_feedback_cases)
